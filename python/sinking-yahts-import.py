...
async def check_domain_batch(session, domains: List[str], bearer_token: str) -> List[Dict]:
    """Check a batch of domains against phish.directory API"""
    ...
    return await asyncio.gather(*[check_single_domain(domain) for domain in domains])
...
async def main():
    bearer_token = "eyINSERT YOUR TOKEN"
